...
func (self StringType) Fields(fieldSpan errors.Span) map[string]Type {
	return map[string]Type{
		...
		"substring": NewFunctionType(
			NewNormalFunctionTypeParamKind([]FunctionTypeParam{NewFunctionTypeParam(ast.NewSpannedIdent("upper", fieldSpan), NewIntType(fieldSpan), nil)}),
			fieldSpan,
			NewStringType(fieldSpan),
			fieldSpan,
		),
...
func (self FunctionType) SetSpanAdvanced(span errors.Span, paramsSpan errors.Span) Type {
	...
	if self.ReturnType != nil {
		returnType = self.ReturnType.SetSpan(span)
	}
	return NewFunctionType(self.Params, paramsSpan, returnType, span)
}
