...
// Interpreter code
func (self *Interpreter) visitStatements(statements []StatementOrExpr) (Result, *int, *errors.Error) {
	/*
		SIGTERM catching
		Pre-execution validation of potential sigTerm checks if the function has to be aborted
		If a signal is received, the current function aborts using the provided exit-code
...
func (self *Interpreter) visitTryExpression(node AtomTry) (Result, *int, *errors.Error) {
	...
	if err != nil {
		...
		catchResult, code, err := self.visitStatements(node.CatchBlock.IntoItemsList())
		if code != nil || err != nil {
			return Result{}, code, err
		}
...
func (self *Interpreter) visitIfExpression(node IfExpr) (Result, *int, *errors.Error) {
	...
	if conditionIsTrue {
		...
		value, code, err := self.visitStatements(node.Block.IntoItemsList())
		if code != nil || err != nil {
			return Result{}, code, err
		}
		self.popScope()
	...
	value, code, err := self.visitStatements((*node.ElseBlock).IntoItemsList())
	if code != nil || err != nil {
		return Result{}, code, err
	}
	self.popScope()
...
func (self *Interpreter) visitForExpression(node AtomFor) (Result, *int, *errors.Error) {
	...
	for {
		...
		value, code, err := self.visitStatements(node.IterationCode.IntoItemsList())
		if code != nil || err != nil {
			return Result{}, code, err
		}
...
func (self *Interpreter) visitWhileExpression(node AtomWhile) (Result, *int, *errors.Error) {
	...
	for {
		...
		result, code, err := self.visitStatements(node.IterationCode.IntoItemsList())
		if code != nil || err != nil {
			return Result{}, code, err
		}
...
func (self *Interpreter) callValue(span errors.Span, value Value, args []Expression) (Value, *int, *errors.Error) {
	...
	case TypeFunction:
		...
		returnValue, code, err := self.visitStatements(function.Body.IntoItemsList())
		if code != nil || err != nil {
			return nil, code, err
		}
